...
/*NV_Patch: (NV_BugID:1512615) -- Start --*/
	bool dvfs_disabled;
/*NV_Patch: (NV_BugID:1512615) -- End --*/
	int speedo;
};
...
//NV_Patch: (NV_BugID:1512615): Ignore err if dvfs overrides are disabled
/* 
    If dvfs overrides are disabled, ignore the error values returned
    by dvfs override APIs and continue tuning execution rather than
    aborting tuning 
*/
...
/*NV_Patch: (NV_BugID:1512615) -- Start --*/
      if (tegra_host->dvfs_disabled) {
         pr_debug("DVFS is diasbled, assigning vmin to boot voltage\n");
         curr_vmin = tegra_host->boot_vcore_mv;
      }
      vmin = curr_vmin;
/*NV_Patch: (NV_BugID:1512615) -- End --*/
...
*/
	...
	if ((err == -EPERM) || (err == -ENOSYS)) {
		dev_err(mmc_dev(sdhci->mmc), "dvfs overrides disabled. Vmin not updated\n");
		err = 0;
	}
#endif
...
*/
	...
	if ((err == -EPERM) || (err == -ENOSYS)){
		dev_err(mmc_dev(sdhci->mmc),
			"dvfs overrides disabled. Nothing to clear\n");
		err = 0;
	}
#endif
...
/*NV_Patch: (NV_BugID:1512615) -- Start --*/
//pr_err("+++++%s Vcore voltage = %d\n", mmc_hostname(sdhci->mmc), regulator_get_voltage(tegra_host->vcore_reg));
/*NV_Patch: (NV_BugID:1512615) -- End --*/
...
*/
	 ...
	 * continue execution rather than failing tuning altogether.
	 */
 	err = tegra_dvfs_override_core_voltage(pltfm_host->clk, voltage);
	if ((err == -EPERM) || (err = -ENOSYS)){
		dev_err(mmc_dev(sdhci->mmc),
			"dvfs overrides disabled. No overrides set\n");
		err = 0;
	} else if (err) {
 		dev_err(mmc_dev(sdhci->mmc),
 			"failed to set vcore override %dmv\n", voltage);
...
/*NV_Patch: (NV_BugID:1512615) -- Start --*/
       tegra_host->vcore_reg = devm_regulator_get(mmc_dev(host->mmc), "vdd_core");
/*NV_Patch: (NV_BugID:1512615) -- End --*/
...
/*NV_Patch: (NV_BugID:1512615) -- Start --*/
//#if !defined(CONFIG_TEGRA_CPU_DVFS)
      tegra_host->dvfs_disabled = true;
//#endif
/*NV_Patch: (NV_BugID:1512615) -- End --*/
