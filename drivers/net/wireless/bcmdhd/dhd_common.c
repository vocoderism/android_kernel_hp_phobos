...
{
//Use SKU to determine which NVRAM will be downloaded to wifi module 
	printf("dhd_common_init--Start to copy FW and NVRAM path\n");
	...
	printf("dhd_common_init--fw_path= %s\n", fw_path);
...
#else
     printf("dhd_common_init-- NVRAM path\n");
     {
	      void * fp_sku = NULL;
		//Compiler seems have something wrong while complie char array with element number smaller than size 8.
		//Problem--> Unexpected string will fill out char array even the size of array is smaller than string size.
		//Char array size extend to 8 or larger will avoid this problem.
		char country_code[8]; //country_code[2];  
		char psku_path[10];
	       memset(psku_path,0,10);
	       memset(country_code,0,8);
              strcpy(psku_path,"/data/.sku");
		fp_sku = dhd_os_open_image(psku_path);
		if (fp_sku == NULL){
                  printf("dhd_common_init-- Can't read %s\n", psku_path);
		     dhd_os_close_image(fp_sku);
		     bcm_strncpy_s(nv_path, sizeof(nv_path), "/data/misc/wifi/firmware/nvram_us.txt", MOD_PARAM_PATHLEN-1);
		}else{
                  dhd_os_get_image_block(country_code, 2, fp_sku);
                  printf("dhd_common_init-- country_code: %s\n", country_code);  
#ifdef CONFIG_BCMDHD_NVRAM_PATH
		    if(strcmp(country_code, "US") == 0){
		         bcm_strncpy_s(nv_path, sizeof(nv_path), "/data/misc/wifi/firmware/nvram_us.txt", MOD_PARAM_PATHLEN-1);
		    }else if(strcmp(country_code, "ID") == 0){
		         bcm_strncpy_s(nv_path, sizeof(nv_path), "/data/misc/wifi/firmware/nvram_id.txt", MOD_PARAM_PATHLEN-1);
	          }else{
		         bcm_strncpy_s(nv_path, sizeof(nv_path), "/data/misc/wifi/firmware/nvram_ww.txt", MOD_PARAM_PATHLEN-1);
		   }
#else /* CONFIG_BCMDHD_NVRAM_PATH */
		       nv_path[0] = '\0';
#endif /* CONFIG_BCMDHD_NVRAM_PATH */
...
#endif
	printf("dhd_common_init--nv_path= %s\n", nv_path);
