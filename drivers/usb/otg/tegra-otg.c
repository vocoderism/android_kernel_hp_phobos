...
int otg_wake_irq=0; //52
//int otg_wake_irq_2=118;

int usb1_status=0;  // 1 ->HOST 2->ADB
#define TEGRA_GPIO_PK4		84
#if Enable_CLOSE_5V
#define DISABLE_IRQ_C5V 1
#else
#define DISABLE_IRQ_C5V 0
#endif
				...
				struct usb_gadget *gadget)
{
	...
	if (g_usb1_ready == 1)
	{
	        printk("[OTG] skip USB1 to ADB \r\n");
		return 0;
	}
	
	if (g_adb_init ==1) return 0;
	else g_adb_init=1;
	
	usb1_status=2;
        printk("[OTG] OTG-ADB [%d]\r\n",usb1_status);
...
static int tegra_otg_set_host(struct usb_otg *otg, struct usb_bus *host)
{
	...
	if (g_usb1_ready == 1)
	{
	        printk("[OTG] skip USB1 to Host \r\n");
		return 0;
	}
	
	if (g_host_init ==1) return 0;
	else g_host_init=1;		

        
	usb1_status=1;
	printk("[OTG] OTG-HOST[%d] \r\n",usb1_status);
...
static int tegra_otg_suspend(struct device *dev)
{
	...
	tegra->suspended = true;
         ...
         if(usb1_status == 2)
         {
	     if(otg_wake_irq != 0 && g_adb_disable_irq ==0)
	     {
	     		 disable_irq_wake(otg_wake_irq);
			  g_adb_disable_irq=1;
	     }
	      msleep(10);	 
	      enable_interrupt(tegra, false);	
	      printk("[OTG]2 otg_wake_irq to stop[%d] \r\n",otg_wake_irq); 	 
	      msleep(10);	
	      gpio_set_value(TEGRA_GPIO_PK4, 1);		  
         }
	else if(usb1_status ==1)
	{
	     //if(otg_wake_irq != 0)
	      		// disable_irq_wake(otg_wake_irq);
		 
	      enable_interrupt(tegra, false);	
	      printk("[OTG]1 otg_wake_irq to stop[%d] \r\n",otg_wake_irq); 	 
...
static void tegra_otg_resume(struct device *dev)
{
	...
	if(usb1_status ==1)
	{
	     ...
	     gpio_set_value(TEGRA_GPIO_PK4, 0);
             msleep(10);
               //if(otg_wake_irq != 0)
                //        enable_irq_wake(otg_wake_irq);
	        //printk("[OTG]1 otg_wake_irq to start[%d] \r\n",otg_wake_irq); 	 		   
	}
	else if(usb1_status == 2)
	{
		...
		//if(otg_wake_irq != 0)
		//		 enable_irq_wake(otg_wake_irq);
	        //printk("[OTG]2 otg_wake_irq to start[%d] \r\n",otg_wake_irq); 
	}
#endif			   
