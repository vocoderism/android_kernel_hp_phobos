...
static int tegra_ehci_resume(struct platform_device *pdev)
{
	struct tegra_ehci_hcd *tegra = platform_get_drvdata(pdev);
	   ...
	   if(usb1_status ==2)
	   {
			...
			printk("[ECHI]enable_irq_wake[%d]\r\n \n",tegra->irq);  
	   }
	   else if(usb1_status ==1)
	   {
			 if (tegra->irq == 52)
				   enable_irq_wake(tegra->irq);	
			printk("[ECHI] H enable_irq_wake[%d]\r\n \n",tegra->irq);  
	   }	   
       #endif 
...
static int tegra_ehci_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (tegra->bus_suspended_fail)
		return -EBUSY;
                ...
                //printk("[WOLO] tegra_ehci_suspend [%d]!!!\r\n",pdata->port_otg);
		...
		if(regen2==0 && ret==0)
		{
			...
			printk("[ECHI]disable_irq_wake[%d]\r\n \n",tegra->irq); 	
		     }
		     else  if(usb1_status ==1)
		     {
	                 if (tegra->irq ==52)
		                   disable_irq_wake(tegra->irq);	
			printk("[ECHI]H disable_irq_wake[%d]\r\n \n",tegra->irq); 		     
		     }
		     #endif	
		     // dev_err(&pdev->dev, "call the [palmas_suspend_regen2] function \n"); 
		     // palmas_suspend_regen2();
		     regen2=1;			 
		}
		//if(pdata->port_otg == 1) go_to_sleep=1;
	//regen2=1;	
	return ret;
#else		
