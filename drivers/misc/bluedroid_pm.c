...
static int bluedroid_pm_probe(struct platform_device *pdev)
{
	...
	if (res) {
		...
		gpio_direction_output(bluedroid_pm->ext_wake, 0);  // BT device wake: set low active
					...
					unsigned long count, void *data)
{
	...
	if (!bluedroid_pm->is_blocked) {
		if (buf[0] == '0') {
			...
			gpio_set_value(bluedroid_pm->ext_wake, 1); // BT device wake: set low active
			wake_unlock(&bluedroid_pm->wake_lock);
		} else if (buf[0] == '1') {
			//gpio_set_value(bluedroid_pm->ext_wake, 1);
			gpio_set_value(bluedroid_pm->ext_wake, 0);  // BT device wake: set low active
