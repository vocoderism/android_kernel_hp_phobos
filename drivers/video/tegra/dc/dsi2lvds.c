...
static u8 dsi2lvds_config_clk[][2] = {
	...
	DSI2LVDS_REG_VAL(0x0a,0x03), /* configure PLL */
...
static u8 dsi2lvds_config_clkA[][2] = {
#if Foxconn_SN65DSI85_Bridge
	DSI2LVDS_REG_VAL(0x0d, 0x00), /* pLL disable */
...
static u8 dsi2lvds_config_dsi[][2] = {
	...
	DSI2LVDS_REG_VAL(0x12,0x40), /* channel A clk range */
...
static u8 dsi2lvds_config_dsiA[][2] = {
#if Foxconn_SN65DSI85_Bridge
	DSI2LVDS_REG_VAL(0x10,0x26), /* default left right ganged mode */
...
static u8 dsi2lvds_config_video[][2] = {
	...
	DSI2LVDS_REG_VAL(0x21, 0x06),
	DSI2LVDS_REG_VAL(0x22, 0x40), /* horizontal pixels on dsi channel B */
	DSI2LVDS_REG_VAL(0x23, 0x06),
	DSI2LVDS_REG_VAL(0x24, 0x84), /* vertical pixels on lvds channel A */
	DSI2LVDS_REG_VAL(0x25, 0x03),
	DSI2LVDS_REG_VAL(0x26, 0x84), /* vertical pixels on lvds channel B */
	DSI2LVDS_REG_VAL(0x27, 0x03),
...
static u8 dsi2lvds_config_videoA[][2] = {
#if Foxconn_SN65DSI85_Bridge
...
#endif	 
	gpio_request(lvds_en, "lvds enable");
...
static void tegra_dsi2lvds_enable(struct tegra_dc_dsi_data *dsi)
{
	...
	if(k==0){
		...
		err = dsi2lvds_i2c_transfer(dsi2lvds, dsi2lvds_config_clkA,
				ARRAY_SIZE(dsi2lvds_config_clkA), I2C_WRITE);
		if (err < 0) {
			dev_err(&dsi->dc->ndev->dev, "dsi2lvds: Init clk failed\n");
			goto fail;
		}
		...
		if (dsi2lvds->dsi->info.ganged_type ==
			...
			u32 cnt;
			for (cnt = 0;
			cnt < ARRAY_SIZE(dsi2lvds_config_dsiA); cnt++) {
				if (dsi2lvds_config_dsiA[cnt][0] == 0x10)
				/* select odd even ganged mode */
					dsi2lvds_config_dsiA[cnt][1] &= ~(1 << 7);
			}
		}

		err = dsi2lvds_i2c_transfer(dsi2lvds, dsi2lvds_config_dsiA,
				ARRAY_SIZE(dsi2lvds_config_dsiA), I2C_WRITE);
		if (err < 0) {
			dev_err(&dsi->dc->ndev->dev, "dsi2lvds: Init dsi failed\n");
			goto fail;
		}
		...
		err = dsi2lvds_i2c_transfer(dsi2lvds, dsi2lvds_config_videoA,
				ARRAY_SIZE(dsi2lvds_config_videoA), I2C_WRITE);
		if (err < 0) {
			dev_err(&dsi->dc->ndev->dev, "dsi2lvds: Init video failed\n");
			goto fail;
		}		
...
{
	//pr_warn("==================tegra_dsi2lvds_suspend==================\n");
	dsi2lvds->dsi2lvds_enabled = false;
	gpio_direction_output(lvds_en, 1);
	gpio_set_value(lvds_en, 0);
	/* To be done */
